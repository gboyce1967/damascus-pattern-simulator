...
|-- vispy_3d_viewer.py                   # VisPy OpenGL viewer for GUI
...
|-- Debug outputs (generated by exports or demos):
...
- `Installation_and_Launch/damascus_simulator.spec` now packages assets from:
  - `data/`
  - `Staging/`
- Runtime logs now write into `logs/` at the project root

### GUI Visualization Migration (2026-02-07)

The Tkinter GUI now renders 3D content through VisPy (`vispy_3d_viewer.py`)
instead of matplotlib's embedded 3D axes.

**Key changes:**
- Added `DamascusVispyViewer` integration in `damascus_3d_gui.py`
- Enabled proper interactive rotate/pan/zoom camera controls in the GUI
- Kept cross-section PNG export, but removed the live cross-section preview panel
- Kept simulator-side matplotlib visualization helpers for standalone/debug use
...
### 3D Visualization (VisPy in GUI)

**Why VisPy for the GUI viewport:**
- Proper zoom behavior with OpenGL camera controls
- Better interaction quality for rotate/pan/zoom in Tkinter
- Dedicated viewer module: `vispy_3d_viewer.py`

**Note on simulator-level rendering:**
- `damascus_3d_simulator.py` still contains matplotlib visualization helpers
- Those helpers are useful for standalone/debug workflows outside the GUI
...
**Rendering Details:**
- Each layer mesh is passed from Open3D to a VisPy mesh visual
- Build plate boundary is rendered as a line visual
- Camera uses turntable-style interaction for intuitive inspection

### Cross-Section Views

**Display:**
- Live cross-section panel is currently disabled in the GUI
- Cross-section export remains available via "Save Cross-Section (PNG)"
...
**3D Visualization:**
- Rendering: <1s
- Interactive framerate: 30-60 FPS (VisPy/OpenGL, hardware dependent)
...
- **Cause**: OpenGL/driver/backend initialization issues
- **Solution**: Launch via `run_windows.bat` and verify installer dependencies were completed
- **Debug**: Check `logs/damascus_3d_debug_*.log` for VisPy/OpenGL errors
...
3. âœ… Stabilized GUI visualization path with VisPy integration
...
- Use VisPy for GUI viewport rendering; keep matplotlib helper paths in simulator code
...
- **VisPy**: GUI 3D viewport rendering in Tkinter
- **PyOpenGL / pyopengltk**: OpenGL bridge dependencies for VisPy + Tkinter
- **Matplotlib 3.10.8**: optional plotting/standalone visualization helpers
...
### Version 2.1.1-beta (2026-02-07) - VISPY GUI INTEGRATION + DOC REFRESH
- Switched GUI 3D viewport to VisPy (`vispy_3d_viewer.py`) for proper zoom/pan/rotate behavior
- Updated GUI layout to prioritize full-height 3D view; live cross-section preview panel removed
- Kept PNG cross-section export path and simulator-side plotting helpers
- Confirmed runtime debug log path as `logs/damascus_3d_debug_*.log`
- Updated docs for current folder structure and launcher/installer behavior
...
- Legacy approach retired from active development branch
